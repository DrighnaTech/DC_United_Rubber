        ...
        tooltip: {
          callbacks: {
            label: ctx => {
              ...
              return ` ${ctx.label}: ${fmtINRFull(ctx.raw)} — ${pct}%`;
            },
          },
        },
        datalabels: {
...
/** Download the active tab as PDF using browser's native print (vector text — crisp) */
function exportPDF() {
  // Hide everything except the active tab, then trigger browser print dialog
  const tabEl = document.getElementById('tab-' + activeTab);
  if (!tabEl) return;

  // Create print-only wrapper
  ...
  tabEl.querySelectorAll('canvas').forEach(c => {
    if (c.id) {
      try { canvasData[c.id] = c.toDataURL('image/png'); } catch (_) {}
    }
  });
...
@media print {
  ...
  .pagination, .filter-bar, .site-header, .tab-bar { display: none !important; }
}
body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #fff; margin: 0; padding: 16px; }
.print-header { background: #1A3C5E; color: #fff; padding: 12px 20px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center; border-radius: 6px; }
.print-header h2 { margin: 0; font-size: 16px; }
.print-header span { font-size: 11px; opacity: 0.7; }
</style>
</head><body>
