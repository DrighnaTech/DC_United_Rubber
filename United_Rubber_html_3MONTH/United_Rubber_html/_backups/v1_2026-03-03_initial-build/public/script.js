...
function updateFilterBadge() {
  ...
  const active = [status, type].filter(Boolean);
...
async function loadSalesDashboard() {
  ...
  const { kpi, status, monthly, customers } = data;

  // ── KPI Cards
  document.getElementById('sd-total-sales').textContent    = fmtINR(kpi.total_sales);
...
async function loadSalesMap() {
  ...
  const { states, districts, zones, summary } = data;
...
function renderDistrictTable(rows) {
  ...
  if (!rows.length) { tbody.innerHTML = '<tr><td colspan="5" class="table-empty">No district data available</td></tr>'; return; }
...
async function loadSalesAnalysis() {
  ...
  const { monthly, yearly, mom, typeTrend, statusTrend } = data;
