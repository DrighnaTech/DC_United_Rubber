...
async function loadSalesDashboard() {
  ...
  document.getElementById('sd-total-rate').textContent   = '₹' + parseFloat(kpi.total_rate_cr || 0).toFixed(2) + ' Cr';
...
async function loadSalesAnalysis() {
  ...
  if (monthly.length) {
    const sorted = [...monthly].sort((a, b) => a.month_key.localeCompare(b.month_key));
    ...
    document.getElementById('sa-from-date').textContent  = first.month_label;
    document.getElementById('sa-to-date').textContent    = last.month_label;
