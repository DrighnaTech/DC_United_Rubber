...
function clearFilters() {
  ['filterStatus', 'filterInvoiceType', 'filterSite', 'filterShipState'].forEach(id => {
    const dropdown = document.getElementById('ms-dropdown-' + id);
...
async function loadSalesDashboard() {
  ...
  document.getElementById('sd-avg-rate').textContent  = '₹' + parseFloat(kpi.avg_rate || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
async function loadSalesAnalysis() {
  ...
  const { monthly, yearly, mom, typeTrend, statusTrend, siteTrend } = data;
