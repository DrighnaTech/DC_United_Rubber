...
router.get('/', async (req, res) => {
  try {
    ...
    const [kpiRes, statusRes, monthlyRes, customerRes, itemCatRes, itemKpiRes] = await Promise.all([
      ...
      // ── Total Sales Qty + Avg Rate (item table, filtered invoices) ─
      db.query(
        `${cte},
         filtered_inv AS (
           SELECT "${C.invoiceNo}" FROM deduped ${postFilter}
         ...
         ${itemCTE}
         SELECT
           ROUND(SUM(COALESCE(NULLIF(di."${C.salesQty}",'')::NUMERIC, 0)), 0) AS total_sales_qty,
           ROUND(AVG(NULLIF(di."${C.rate}",'')::NUMERIC), 2)                   AS avg_rate
         FROM deduped_items di
         INNER JOIN filtered_inv fi ON di."${C.invoiceNo}" = fi."${C.invoiceNo}"`,
        values
      ),
    ...
    kpi.avg_rate        = itemKpi.avg_rate || 0;
