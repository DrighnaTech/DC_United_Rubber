...
function buildParams(extra = {}) {
  ...
  const invoiceType = document.getElementById('filterInvoiceType')?.value || '';
  const dateFrom    = document.getElementById('filterDateFrom')?.value || '';
  const dateTo      = document.getElementById('filterDateTo')?.value || '';
  const site        = document.getElementById('filterSite')?.value || '';
  const shipState   = document.getElementById('filterShipState')?.value || '';
...
/** Fetch JSON from API with loading indicator */
...
async function loadFilterOptions() {
  ...
  const typeSel     = document.getElementById('filterInvoiceType');
  const siteSel     = document.getElementById('filterSite');
  const shipStateSel = document.getElementById('filterShipState');
  ...
  (data.shipState || []).forEach(s => {
    const o = document.createElement('option');
    o.value = s; o.textContent = s;
    shipStateSel.appendChild(o);
  });
}
...
function clearFilters() {
  ...
  document.getElementById('filterShipState').value   = '';
  updateFilterBadge();
  loadTab(activeTab);
}
...
function updateFilterBadge() {
  const status    = document.getElementById('filterStatus')?.value || '';
  const type      = document.getElementById('filterInvoiceType')?.value || '';
  const dateFrom  = document.getElementById('filterDateFrom')?.value || '';
  const dateTo    = document.getElementById('filterDateTo')?.value || '';
  const site      = document.getElementById('filterSite')?.value || '';
  const shipState = document.getElementById('filterShipState')?.value || '';
  const active = [status, type, dateFrom, dateTo, site, shipState].filter(Boolean);
  const ind    = document.getElementById('filterIndicator');
  if (active.length) {
    ind.style.display = 'flex';
    document.getElementById('filterBadge').textContent =
      active.length + ' filter' + (active.length > 1 ? 's active' : ' active');
  } else {
...
async function loadSalesDashboard() {
  ...
  const { kpi, status, monthly, customers, itemCategory } = data;

  // ── KPI Cards
  document.getElementById('sd-total-sales').textContent      = fmtINR(kpi.total_net_amount);
  document.getElementById('sd-gross-amount').textContent     = fmtINR(kpi.total_gross_amount);
  document.getElementById('sd-total-tax').textContent        = fmtINR(kpi.total_tax);
  document.getElementById('sd-total-invoices').textContent   = fmtNum(kpi.total_invoices);
  document.getElementById('sd-avg-value').textContent        = 'Avg: ' + fmtINR(kpi.avg_invoice_value);
  document.getElementById('sd-unique-customers').textContent = fmtNum(kpi.unique_customers);
  document.getElementById('sd-avg-inv').textContent          = fmtINR(kpi.avg_invoice_value);
  ...
  // ── Status Pie Chart
  renderStatusPie('sd-status-chart', status);
...
async function loadSalesMap() {
  ...
  const { states, cities, zones, summary } = data;
...
async function loadInvoiceSummary() {
  ...
  const { kpi, statusBreakdown, typeBreakdown, monthly } = data;
  ...
  document.getElementById('is-max').textContent       = fmtINR(kpi.max_amount);
  ...
  // ── Status Bar Chart
  renderStatusBarChart('is-status-chart', statusBreakdown);

  // ── Type Pie Chart
  renderTypePieChart('is-type-chart', typeBreakdown);

  // ── Monthly Invoice Count Line
  renderMonthlyCountChart('is-monthly-chart', monthly);
...
async function loadSalesAnalysis() {
  ...
  const { monthly, yearly, mom, typeTrend, statusTrend, siteTrend } = data; // eslint-disable-line no-unused-vars

  // ── Yearly KPI Cards
  renderYearlyKPIs(yearly);
  ...
  // ── Type Trend Stacked Bar
  renderTypeTrendChart(typeTrend);

  // ── Status Trend Line
  renderStatusTrendChart(statusTrend);

  // ── MoM Table
  renderMomTable(mom, monthly);
}
