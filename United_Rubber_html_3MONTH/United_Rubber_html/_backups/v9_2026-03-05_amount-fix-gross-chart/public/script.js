...
function renderSiteBarChart(rows) {
  ...
  // Net Amount datasets (stack: 'net') — solid colors
  const netDatasets = sites.map((site, i) => ({
    label: site + ' (Net)',
    stack: 'net',
    data: months.map(mk => {
  ...
  // Gross Amount datasets (stack: 'gross') — lighter shade
  const grossDatasets = sites.map((site, i) => ({
    label: site + ' (Gross)',
    stack: 'gross',
    data: months.map(mk => {
      const r = rows.find(r => r.month_key === mk && r.site === site);
    ...
    backgroundColor: siteColors[i % siteColors.length] + '88',
    borderColor: siteColors[i % siteColors.length],
    borderWidth: 1,
    borderRadius: 3,
    borderSkipped: false,
  }));
  ...
  charts[id] = new Chart(ctx, {
    type: 'bar',
    data: { labels: monthLabels, datasets: [...netDatasets, ...grossDatasets] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => {
              const type = ctx.dataset.stack === 'net' ? 'Net' : 'Gross';
              return ` ${ctx.dataset.label.replace(/ \(Net\)| \(Gross\)/,'')}: ${fmtINRFull(ctx.raw)} (${type})`;
            }
          }
        },
      },
