...
/** Debounced handler for Customer Name text input (avoids DB query on every keystroke) */
let _custNameTimer = null;
function onCustomerNameChange() {
  clearTimeout(_custNameTimer);
  _custNameTimer = setTimeout(() => onFilterChange(), 400);
}
...
function clearFilters() {
  ...
  const custEl = document.getElementById('filterCustomerName');
...
function switchTab(tabId, btnEl) {
  ...
  // Leaflet map must be notified when its container becomes visible
...
async function initLeafletMap(stateData) {
  ...
  // Ensure Leaflet redraws correctly after tab becomes visible
