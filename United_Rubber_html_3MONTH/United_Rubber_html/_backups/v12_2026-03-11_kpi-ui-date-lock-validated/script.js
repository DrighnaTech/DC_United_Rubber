...
// Register Chart.js datalabels plugin globally (disabled by default — enable per chart)
...
function buildParams(extra = {}) {
  const status       = getMultiSelectValues('filterStatus') || 'Exported To GL';
  ...
  const dateFrom     = document.getElementById('filterDateFrom')?.value || defaultDateFrom;
  const dateTo       = document.getElementById('filterDateTo')?.value || defaultDateTo;
...
/** Format Indian currency — full ₹ with Indian locale (for tables & general display) */
function fmtINR(val) {
  const n = parseFloat(val) || 0;
  return '₹' + n.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

/** ₹XX.XX Cr format — for chart data labels and axis ticks */
function fmtCr(val) {
  const n = parseFloat(val) || 0;
  return '₹' + (n / 1e7).toFixed(2) + ' Cr';
}

/** Detailed ₹ for tooltips — full Indian locale number */
function fmtINRFull(val) {
  const n = parseFloat(val) || 0;
  return '₹' + n.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

/** KPI display — full detailed ₹ with Indian locale */
...
function showLoader(state) {
  ...
  if (state) {
    ...
    // Safety: auto-hide after 8s in case of stuck state
    clearTimeout(_loaderTimer);
    _loaderTimer = setTimeout(() => { el.style.display = 'none'; }, 8000);
  } else {
...
/* ═══════════════════════════════════════════════
   FILTERS
═══════════════════════════════════════════════ */
// Store default date range from DB (used by clearFilters)
let defaultDateFrom = '2024-04-01';
let defaultDateTo   = '2024-06-30';

async function loadFilterOptions() {
  const data = await apiFetch('/api/filters');
  if (!data) return;
  buildMultiSelect('filterStatus',      data.status      || [], ['Exported To GL']);
  ...
  // Set default date range from DB min/max
  if (data.dateRange) {
    defaultDateFrom = data.dateRange.min_date || '';
    defaultDateTo   = '2024-06-30';
    const fromEl = document.getElementById('filterDateFrom');
    const toEl   = document.getElementById('filterDateTo');
    fromEl.value = defaultDateFrom;
    toEl.value   = defaultDateTo;
    // Lock calendar: no dates before DB min or after Jun 2024
    if (defaultDateFrom) {
      fromEl.min = defaultDateFrom;
      toEl.min   = defaultDateFrom;
    }
  }
}

/** Called whenever a filter changes — snap cleared dates back to defaults */
function onFilterChange() {
  const fromEl = document.getElementById('filterDateFrom');
  const toEl   = document.getElementById('filterDateTo');
  if (!fromEl.value && defaultDateFrom) fromEl.value = defaultDateFrom;
  if (!toEl.value   && defaultDateTo)   toEl.value   = defaultDateTo;
  ...
  ['filterStatus', 'filterInvoiceType', 'filterSite', 'filterShipState', 'filterCustomerName'].forEach(id => {
    ...
    if (dropdown) {
      dropdown.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        // Re-apply default: filterStatus → only "Exported To GL" checked
        cb.checked = (id === 'filterStatus' && cb.value === 'Exported To GL');
      });
...
function updateFilterBadge() {
  ...
  if (dateFrom && dateFrom !== defaultDateFrom) count++;
  if (dateTo   && dateTo   !== defaultDateTo)   count++;
  ...
  charts[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: {
          grid: { display: false },
          ticks: { callback: v => fmtCr(v), font: { size: 11 } },
        },
...
function renderItemCategoryChart(canvasId, rows) {
  ...
  charts[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { grid: { display: false }, ticks: { callback: v => fmtCr(v), font: { size: 10 } } },
...
function renderStateTable(rows) {
  ...
  tbody.innerHTML = rows.map((r, i) => `
    <tr>
      ...
      <td class="text-right">${fmtNum(r.avg_amount)}</td>
    </tr>
  `).join('');
}
  ...
  charts[id] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => `  Qty: ${fmtNum(ctx.raw)}  |  Amt: ${fmtINRFull(validRows[ctx.dataIndex]?.total_amount)}`,
          },
        },
        datalabels: {
          display: true, anchor: 'end', align: 'end', offset: 2,
...
async function initLeafletMap(stateData) {
  ...
  geojsonLayer = L.geoJSON(indiaGeoJson, {
    ...
    onEachFeature: (feature, layer) => {
        ...
        click() {
          const content = row
            ? `<div class="map-popup-title">${name}</div>
               <div class="map-popup-row"><span>Revenue</span><span class="map-popup-val">${fmtINRFull(row.total_amount)}</span></div>
               <div class="map-popup-row"><span>Invoices</span><span class="map-popup-val">${fmtNum(row.invoice_count)}</span></div>
               <div class="map-popup-row"><span>Avg Invoice</span><span class="map-popup-val">${fmtINRFull(row.avg_amount)}</span></div>`
      ...
      if (row) {
        layer.bindTooltip(
          `<strong>${name}</strong><br>${fmtINRFull(row.total_amount)}`,
  ...
  // Add data labels (₹ Cr) on map for states with data
  if (window._mapLabels) window._mapLabels.forEach(m => leafletMap.removeLayer(m));
  window._mapLabels = [];
  geojsonLayer.eachLayer(layer => {
    const name = normaliseState(getGeoStateName(layer.feature));
    const row = stateLookup[name];
    if (row && parseFloat(row.total_amount) > 0) {
      const center = layer.getBounds().getCenter();
      const label = L.marker(center, {
        icon: L.divIcon({
          className: 'map-data-label',
          html: `<span>${fmtCr(row.total_amount)}</span>`,
          iconSize: [80, 20],
          iconAnchor: [40, 10],
        }),
        interactive: false,
  ...
  charts[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y1: { position: 'left', ticks: { callback: v => fmtCr(v), font: { size: 11 } }, grid: { display: false } },
...
function renderMonthlyCountChart(canvasId, rows) {
  ...
  charts[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { callbacks: { label: ctx => '  Count: ' + fmtNum(ctx.raw) } },
        datalabels: {
...
async function loadSalesAnalysis() {
  ...
  if (monthly.length) {
    ...
    // From/To Date: use active filter values, fall back to locked defaults
    const fromDate = document.getElementById('filterDateFrom')?.value || defaultDateFrom;
    const toDate   = document.getElementById('filterDateTo')?.value   || defaultDateTo;
    ...
    document.getElementById('sa-net-amount').textContent  = fmtCr(totalNet);
    document.getElementById('sa-gross-amount').textContent = fmtCr(totalGross);
  }
    ...
    options: {
      ...
      plugins: {
        ...
        datalabels: {
          display: ctx => ctx.datasetIndex === 0,
          anchor: 'end', align: 'top', offset: 4,
          font: { size: 9, weight: 'bold' }, color: '#1A3C5E',
          formatter: v => fmtCr(v),
        },
      },
      scales: {
        x: { grid: { display: false }, ticks: { font: { size: 10 }, maxRotation: 45 } },
        y1: { position: 'left', grace: '15%', ticks: { callback: v => fmtCr(v), font: { size: 11 } }, grid: { display: false } },
        y2: { position: 'right', ticks: { font: { size: 11 } }, grid: { display: false } },
      },
    },
  });
...
function renderSiteBarChart(rows) {
  ...
  const siteColors = ['#1A3C5E','#D97706','#16A34A','#7C3AED','#DC2626','#0891B2'];
    ...
    options: {
      ...
      plugins: {
        ...
        datalabels: {
          ...
          font: { size: 8, weight: 'bold' }, color: '#1A3C5E',
          formatter: (v, context) => {
            const idx = context.dataIndex;
            let total = 0;
            context.chart.data.datasets.forEach(ds => { total += ds.data[idx] || 0; });
            return fmtCr(total);
          },
        },
      },
      scales: {
        ...
        y: { stacked: true, ticks: { callback: v => fmtCr(v), font: { size: 11 } }, grid: { display: false } },
      },
    },
  });
}
  ...
  charts[id] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        datalabels: {
          display: true, anchor: 'end', align: ctx => ctx.raw >= 0 ? 'top' : 'bottom', offset: 2,
    ...
    options: {
      ...
      plugins: {
        ...
        datalabels: {
          ...
          font: { size: 10, weight: 'bold' }, color: '#1A3C5E',
          formatter: v => fmtCr(v),
        },
      },
      scales: {
        x: { grid: { display: false } },
        y: { grid: { display: false }, ticks: { callback: v => fmtCr(v), font: { size: 11 } } },
      },
    },
  });
}
...
function renderTypeTrendChart(rows) {
  ...
  charts[id] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { stacked: true, grid: { display: false }, ticks: { font: { size: 10 }, maxRotation: 45 } },
        y: { stacked: true, grid: { display: false }, ticks: { callback: v => fmtCr(v), font: { size: 11 } } },
      },
    },
  });
}
...
function renderDomesticTable(rows) {
  ...
  // Pivot: site → month_key → { net, gross }
  const pivot = {};
  rows.forEach(r => {
    if (!pivot[r.site]) pivot[r.site] = {};
    pivot[r.site][r.month_key] = {
  ...
  sites.forEach(s => {
    siteTotals[s] = {
      net:   months.reduce((sum, m) => sum + (pivot[s]?.[m]?.net   || 0), 0),
      gross: months.reduce((sum, m) => sum + (pivot[s]?.[m]?.gross || 0), 0),
    };
  });
  const monthTotals = {};
  months.forEach(m => {
    monthTotals[m] = {
      net:   sites.reduce((sum, s) => sum + (pivot[s]?.[m]?.net   || 0), 0),
      gross: sites.reduce((sum, s) => sum + (pivot[s]?.[m]?.gross || 0), 0),
    };
  });
  // Two sub-columns per month (Net Amt / Gross Amt), matching Excel CRD
  const monthHeaders = months.map(m =>
    `<th colspan="2" class="text-center" style="border-left:2px solid var(--border)">${monthLabels[m]}</th>`
  ).join('');
  const subHeaders = months.map(() =>
    `<th class="text-right" style="font-size:10px;color:var(--text-3);border-left:2px solid var(--border)">Net Amt</th>
     <th class="text-right" style="font-size:10px;color:var(--text-3)">Gross Amt</th>`
  ).join('');
  ...
  const tbody = sites.map(s => `<tr>
    ...
    ${months.map(m => `
      <td class="text-right" style="border-left:2px solid var(--border-2)">${fmtCr(pivot[s]?.[m]?.net   || 0)}</td>
      <td class="text-right" style="color:var(--text-2)">${fmtCr(pivot[s]?.[m]?.gross || 0)}</td>`).join('')}
  </tr>`).join('');

  const tfoot = `<tr>
    <td><strong>Total</strong></td>
    ${months.map(m => `
      <td class="text-right fw-bold" style="border-left:2px solid var(--border-2)">${fmtCr(monthTotals[m].net)}</td>
      <td class="text-right fw-bold" style="color:var(--primary-light)">${fmtCr(monthTotals[m].gross)}</td>`).join('')}
  </tr>`;
...
/** Download the active tab as PDF using browser's native print */
function exportPDF() {
  ...
  // Capture chart canvases as images with white background
  const canvasData = {};
  tabEl.querySelectorAll('canvas').forEach(c => {
    if (c.id) {
      try {
        // Create temp canvas with white background (JPEG has no transparency → would be black)
        ...
        canvasData[c.id] = tmp.toDataURL('image/jpeg', 0.92);
      } catch (_) {}
    }
  });

  // Clone content and replace canvases with images
  const clone = tabEl.cloneNode(true);
  clone.querySelectorAll('canvas').forEach(c => {
    if (c.id && canvasData[c.id]) {
      const img = document.createElement('img');
      img.src = canvasData[c.id];
  ...
  clone.querySelectorAll('.pagination, .table-controls, select, button, .filter-bar').forEach(el => el.remove());
...
body {
  ...
  background: #fff; margin: 0; padding: 12px; font-size: 11px; color: #1a1a2e;
  -webkit-print-color-adjust: exact; print-color-adjust: exact;
}
.print-header {
  background: #1A3C5E; color: #fff; padding: 10px 16px; margin-bottom: 12px;
  display: flex; justify-content: space-between; align-items: center; border-radius: 4px;
}
.print-header h2 { margin: 0; font-size: 14px; font-weight: 600; }
.print-header span { font-size: 10px; opacity: 0.7; }

/* Tab Section Header */
.tab-section-header { margin-bottom: 14px; }
.tab-section-header h2 { font-size: 18px; font-weight: 700; color: #1A3C5E; }
.tab-subtitle { font-size: 12px; font-weight: 400; color: #94a3b8; margin-left: 8px; }

/* KPI Grid */
.kpi-grid { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 12px; }
.kpi-card {
  flex: 1; min-width: 120px; background: #f8fafe; border: 1px solid #e2e8f0;
  border-radius: 6px; padding: 8px 12px;
}
.kpi-icon { display: none; }
.kpi-label { font-size: 9px; color: #64748b; text-transform: uppercase; letter-spacing: .5px; font-weight: 600; }
.kpi-value { font-size: 14px; font-weight: 700; color: #1a1a2e; }
.kpi-value--sm { font-size: 12px; }
.kpi-sub { font-size: 9px; color: #94a3b8; }

/* Chart Cards */
.charts-row { display: flex; gap: 10px; margin-bottom: 12px; flex-wrap: wrap; }
.chart-card { flex: 1; min-width: 200px; border: 1px solid #e2e8f0; border-radius: 6px; background: #fff; overflow: visible; page-break-inside: avoid; }
.chart-card--wide { flex: 2; min-width: 300px; }
.chart-card--full { flex: 1 0 100%; }
.chart-header { padding: 8px 12px; border-bottom: 1px solid #e2e8f0; }
.chart-header h3 { font-size: 11px; font-weight: 600; color: #1a1a2e; }
.chart-body { padding: 8px; height: auto !important; overflow: visible !important; }
.chart-body img { width: 100%; height: auto; display: block; }

/* Tables */
.table-card { border: 1px solid #e2e8f0; border-radius: 6px; overflow: hidden; margin-top: 10px; page-break-inside: avoid; }
.table-header { padding: 8px 12px; border-bottom: 1px solid #e2e8f0; }
.table-header h3 { font-size: 11px; font-weight: 600; }
.table-responsive { overflow: visible; }
.data-table { width: 100%; border-collapse: collapse; font-size: 9px; }
.data-table th {
  padding: 5px 6px; text-align: left; font-size: 8px; font-weight: 600;
  color: #64748b; text-transform: uppercase; background: #f8fafe;
  border-bottom: 1px solid #e2e8f0; white-space: nowrap;
}
.data-table td {
  padding: 4px 6px; border-bottom: 1px solid #f1f5f9; color: #1a1a2e;
  white-space: nowrap; max-width: 150px; overflow: hidden; text-overflow: ellipsis;
}
.sort-icon { display: none; }

/* Map tab */
.map-layout { display: flex; gap: 10px; flex-wrap: wrap; }
.map-sidebar { flex: 1; min-width: 200px; }
.map-container { flex: 2; min-width: 300px; min-height: 200px; }
#mapid { height: 250px !important; }

/* Analysis tab */
.analysis-table-wrap { overflow: visible; }

/* Status badges */
.status-badge { padding: 2px 6px; border-radius: 3px; font-size: 8px; font-weight: 600; }
