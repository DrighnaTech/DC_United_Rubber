...
function renderDomesticTable(rows) {
  ...
  const tbody = sites.map(s => `<tr>
    ...
    ${months.map(m => `
      <td class="text-right" style="border-left:2px solid var(--border-2)">${fmtINR(pivot[s]?.[m]?.net   || 0)}</td>
      <td class="text-right" style="color:var(--text-2)">${fmtINR(pivot[s]?.[m]?.gross || 0)}</td>`).join('')}
  </tr>`).join('');

  const tfoot = `<tr>
    <td><strong>Total</strong></td>
    ${months.map(m => `
      <td class="text-right fw-bold" style="border-left:2px solid var(--border-2)">${fmtINR(monthTotals[m].net)}</td>
      <td class="text-right fw-bold" style="color:var(--primary-light)">${fmtINR(monthTotals[m].gross)}</td>`).join('')}
  </tr>`;
