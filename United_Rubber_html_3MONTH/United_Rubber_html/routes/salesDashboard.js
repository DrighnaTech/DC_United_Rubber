...
const {
  ...
  buildDedupCTE, buildTrendCTE, buildItemCTE,
  buildPagination,
  buildOrderBy,
...
router.get('/', async (req, res) => {
  try {
    ...
    // Separate trend CTE for monthly chart — GROUP BY (Invoice_No_, date) matches CRD
    const trend = buildTrendCTE(filters);
    ...
    const [kpiRes, statusRes, monthlyRes, customerRes, itemCatRes, itemKpiRes, custNetRes] = await Promise.all([
      ...
      // ── Monthly Revenue Trend (uses trend CTE — CRD: GROUP BY Invoice_No_ + date) ──
      db.query(
        `${trend.cte}
         SELECT
           ...
           SUM(${AMOUNT_GROSS_EXPR})                        AS total_amount
         FROM deduped
         WHERE "${C.invoiceDate}" IS NOT NULL
           AND "${C.invoiceDate}" != ''
      ...
      db.query(
        `${cte},
         ...
         SELECT
           (SELECT ROUND(SUM(rate_sum), 0)           FROM rate_agg) AS total_rate_val,
    ...
    kpi.total_rate_val  = itemKpi.total_rate_val  || 0;
    ...
    const dataRes = await db.query(
      `${cte}
       SELECT
         ...
         COALESCE("${C.createdDate}", '')              AS created_date,
         COALESCE("${C.approvedDate}", '')             AS approved_date,
         COALESCE("${C.prepDate}", '')                 AS prep_date,
         COALESCE("${C.removalDate}", '')              AS removal_date,
         COALESCE("${C.shipCity}", '')                 AS ship_city
       FROM deduped
       ${postFilter}
       ${orderBy}
       ${limitClause}`,
